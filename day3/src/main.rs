...
*/
static LOWER_BASE: u8 = 'a' as u8;
static UPPER_BASE: u8 = 'A' as u8;

fn get_batch_priority(arr: &Array2<u8>) -> u64 {
    // find the one value in all 3
    //println!("{:?}", arr);
    ...
    for v in sum {
        if v == 3 {
            ...
            if i < LOWER_BASE {
                // uppercase
                score = 27 + (i - UPPER_BASE) as u64;
            } else {
                // lowercase
                score = 1 + (i - LOWER_BASE) as u64;
            }
            println!("3 of {:?}, {:?}", i, score);
            return score;
        }
        i += 1;
    }
...
fn main() {
    ...
    let mut total_pri: u64 = 0;
    let mut group_present = Array2::<u8>::zeros((3, 128));
    let mut group_member = 0;
    let mut stage_two_prio = 0;
    for untrimmed_line in contents.lines() {
        let line = untrimmed_line.trim();

        let mut left_freq: [u8; 128] = [0; 128];
        let mut right_freq: [u8; 128] = [0; 128];
        let mut i = 0;
        for c in line.bytes() {
            if i < line.len() / 2 {
                left_freq[c as usize] += 1;
            } else {
                right_freq[c as usize] += 1;
            }
            i += 1
        }
        ...
        // find duplicate
        for i in 0..128 {
            if left_freq[i] > 0 && right_freq[i] > 0 {
                let c = i as u8;
                let value: u64;
                ...
                if c < LOWER_BASE {
                    // uppercase
                    value = 27 + (c - UPPER_BASE) as u64;
                } else {
                    // lowercase
                    value = 1 + (c - LOWER_BASE) as u64;
                }
                total_pri += value;
            }
        }

        // Part 2, frequencies within group
        if group_member == 0 {
            // find the badge
            stage_two_prio += get_batch_priority(&group_present);
            group_present.fill(0);
        }
        for c in line.bytes() {
            group_present[[group_member, c as usize]] = 1;
        }
    ...
    // find the badge one last time
    stage_two_prio += get_batch_priority(&group_present);

    println!("Stage 1 Total: {:?}", total_pri);
    println!("Stage 2 Total: {:?}", stage_two_prio);
}
